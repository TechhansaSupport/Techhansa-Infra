...
export default function Hero() {
  ...
  const bgImages = [
    '/images/house-pool.jpg',
    '/images/colony-gate.jpg',
    '/images/lake-apartment.jpg',
    ...
    <section id="home" className="relative min-h-screen flex flex-col overflow-hidden bg-slate-50">
      ...
      >
        {bgImages.map((src, index) => (
          <img
            ...
            className={`absolute inset-0 w-full h-full object-cover object-[60%_center] transition-opacity duration-[2000ms] ease-in-out ${
              index === bgIndex ? 'opacity-100 animate-slow-zoom' : 'opacity-0'
            }`}
          />
        ))}
      ...
      <div className="relative z-10 container mx-auto px-4 md:px-8 pt-32 pb-16 flex-1 flex flex-col lg:flex-row items-center justify-between gap-12">
        ...
        <div className="max-w-2xl text-foreground animate-fade-in">
          <h1 className="text-3xl md:text-4xl font-bold leading-tight mb-6 pb-2 tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-emerald to-emerald-light drop-shadow-[0_4px_12px_rgba(16,185,129,0.3)]">
            SHAPING SKYLINES <br /> BUILDING YOUR <br /> DREAMS
          </h1>
          <p className="text-muted text-xl max-w-xl leading-relaxed border-l-4 border-gold pl-6 py-1 mb-10">
            Welcome to Techhansa Infra. We go beyond bricks and concrete to craft legacy spaces—where visionary architecture meets sustainable, client-first living. Discover homes that inspire and commercial spaces that accelerate growth.
          </p>
              ...
              return (
                ...
                >
                  ...
                  <div className="relative w-full h-[55%] overflow-hidden bg-slate-100 group">
                    <img
                      src={property.coverImage || property.image || '/images/house-pool.jpg'}
                      alt={property.name}

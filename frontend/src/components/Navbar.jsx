...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 md:px-8 py-4 glass-nav transition-all duration-300">
      <Link to="/" className="flex items-center gap-3 text-foreground hover:text-blue transition-colors z-50 hover-scale">
        <img src="/logo.png" alt="Techhansa Infra Logo" className="w-12 h-12 md:w-20 md:h-20 rounded-full object-contain shadow-lg border border-slate-200" />
        <span className="font-title font-bold text-xl md:text-3xl tracking-tight text-gold drop-shadow-sm">Techhansa Infra</span>
      </Link>
      ...
      <div 
        className={`fixed inset-0 bg-white z-40 flex flex-col pt-32 px-6 transition-transform duration-300 ease-in-out md:hidden border-l border-slate-100 ${
